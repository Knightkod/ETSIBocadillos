...
import com.sanzfdu.cafeteriaetsib.bl.CheckboxListAdapter;
...
public class Filter extends Fragment {
    ...
    public void search(){
        ...
        if(selected == null||selected.size()==0){
            ...
            if (compr) {
                for(int i = 0;i<lbagg.size();i++) {
                    lbaggString.add(createText(lbagg.get(i)));


                }
                lv.setAdapter(new TextAdapter(getActivity().getApplicationContext(), lbaggString));
    ...
    public List<String> comprBaggs(String lNameBagg){
       /* Collection collection = (Collection)lNameIngr;
        ...
        if(lNameBagg.split(";").length == 1){
            ...
            arrayStringIngr = lNameBagg.split(";");
            arrayStringBagg = arrayStringIngr[0].split(",");//Basta con subdividir el primero. Si ninguno
                                                            //de estos esta en el resto de splits de la lista de ingredientes, ya da
                                                            //igual, porque ningun bocata estara en todas las busquedas
            for (int i = 0; i < arrayStringBagg.length; i++) { //Por cada elemento de la primera parte de la lista
                for (int j = 1; j < arrayStringIngr.length; j++) { //eligeme otra de las partes de la lista 1 a 1
                    for(int k = 0;k <arrayStringIngr[j].split(",").length;k++) { //Y comprueba que cada elemento es o no el bocata
                        ...
                        if (arrayStringIngr[j].split(",")[k].contains(arrayStringBagg[i])) {
    ...
    public String getBagg(String[] sArray){
        String s = "";
